<template>
  <div :class="['fx-optfile-default',customClass]">
    <input type="file" :accept="accept" :multiple="multiple" @change="testchange" hidden ref="refInp">
    <div class="fx-preview-img-box">
      <transition-group name="list-img">
        <template  v-if="targetType=='box'&&imgListShow&&!showFileList">
          <div class="fx-preview-img-img" v-for="(item,index) in File" :key="index">
            <div class="fx-previre-img-actions">
              <img :src="item.preImgUrl" alt="">
              <div class="customArea">
                <slot v-if="$slot['defalut']"></slot>
                <i class="customIconfx-icon-ashbin" v-else @click="deleteImg(index)"></i>
              </div>
            </div>
          </div>
        </template>
      </transition-group>
      <fx-button :type="type" :leftIcon="icon" @click="changeFile" v-if="targetType=='btn'">{{label}}</fx-button>
      <div :class="['fx-optfie-area',{'fx-optfie-area-big':size=='big','fx-opfile-area-active':activeFile}]" v-if="targetType=='box'" ref="refDiv">
        <div class="fx-optfile-area-text" @click="changeFile">
          <div class="fx-optfile-text-pos">
      ...
      <ul>
        <transition-group name="list">
          <li v-for="(item,index) in FileList" :key="index">
            <div>
              <i class="file-iconfx-icon-file"></i>
              <span>{{item.name}}</span>
              <i class="deletefx-icon-ashbin" @click="deleteImg(index)"></i>
            </div>
          </li>
        </transition-group>
      </ul>
