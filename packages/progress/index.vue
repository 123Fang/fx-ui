<template>
  <div class="fx-progress">
    <div v-if="progressType === 'line'" class="fx-progress-default">
      ...
      <div class="fx-progress-status" v-show="!isShowText">
        <span>
          <slot>
            {{ formatPercent.text }}
          </slot>
    ...
    <div
      v-else
      class="fx-progress-default-circle"
    ...
    >
      ...
      <div v-if="isShowText" class="fx-progress-circle-slot" :style="progressDashboardStyle">
        <slot>
          {{ progressCircleText }}
        </slot>
      </div>
...
const backClass = computed(() => {
  return ["fx-progress-back", props.roundCorner ? "fx-progress-b-r" : ""];
});
const speedClass = computed(() => {
  return props.progressType === "line"
    ? [
        "fx-progress-speed",
        props.isAnimation ? "fx-progress-animation" : "",
        props.roundCorner ? "fx-progress-b-r" : "",
      ]
    : [
        "fx-progress-circle",
        props.isAnimation ? "fx-progress-circle-animation" : "",
        props.roundCorner ? "fx-progress-circle-b-r" : "",
      ];
});
let girth = "";
...
.fx-progress-default-circle {
  display: inline-block;
  position: relative;
  .fx-progress-circle-slot {
    position: absolute;
    top: 50%;
    left: 50%;
...
.fx-progress-circle-b-r {
  stroke-linecap: round;
}
.fx-progress-animation {
  animation: infiniteLine 3s ease-out infinite;
}
.fx-progress-circle-animation {
