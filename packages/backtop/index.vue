<template>
  <transition name="fade">
    <div :class="['fx-backtop-box',customClass]" v-if="backtopShow" @click="backtop">
      <slot v-if="slot['default']"></slot>
      <div class="fx-backtop-default-box" v-else>
        <span class="default-backtop-icon">
          <i class="fx-icon-arrow-up-filling"></i>
        </span>
      </div>
    </div>
  </transition>
...
.fx-backtop-box,.fx-backtop-default-box{
  width: auto;
  overflow: auto;
  position: fixed;
